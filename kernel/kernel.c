...
static uint32_t divide_u64_by_1000000000(uint64_t value) {
    uint64_t quotient = 0;
    uint64_t remainder = 0;

    for (int bit = 63; bit >= 0; bit--) {
        remainder = (remainder << 1) | ((value >> bit) & 1u);
        if (remainder >= 1000000000ULL) {
            remainder -= 1000000000ULL;
            quotient |= (1ULL << bit);
        }
    }
...
uint32_t kernel_uptime_seconds(void) {
    uint64_t now = read_tsc();
    if (boot_tsc == 0) {
        boot_tsc = now;
        return 0;
    }
    return divide_u64_by_1000000000(now - boot_tsc);
}
...
static void draw_splash(void) {
    ...
    const char *sub = "METRO-OS  v0.5  BUILD: D34DR00M-1.5.5";
...
static void shell_header(void) {
    ...
    vga_puts("===============================================================\n");
    vga_puts("      METRO-OS 0.5  |  Live Shell\n");
    vga_puts("===============================================================\n");
...
static void run_command(char *buf) {
    ...
    if (kstrcmp(buf, "help") == 0)        shell_help();
        ...
        vga_set_color(COLOR_LCYAN, COLOR_BLACK);
        vga_puts("METRO-OS v0.5 BUILD D34DR00M-1.5.5\n");
        vga_set_color(COLOR_WHITE, COLOR_BLACK);
    }
